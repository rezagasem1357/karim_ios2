...
int _inventoryDaysRemainingForDate(DateTime date) {
  final j = _gregorianToJalali(date.year, date.month, date.day);
  final day = j[2];
...
class _DeliveryScreenState extends State<DeliveryScreen> {
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _eventCountdownRow('انبارگردانی',
                              _inventoryDaysRemainingForDate(now)),
  ...
  String _morningNotificationBody(DateTime now) {
    final remaining = _inventoryDaysRemainingForDate(now);
    final inventoryText = remaining == 0
        ? 'امروز زمان انبارگردانی است.'
        : '${_toPersianDigits(remaining.toString())} روز مانده تا انبارگردانی.';
    final events = <String>[];
  ...
  void _openShareReportChooser() {
    ...
    if (!hasSales && !hasManifests) {
      _showSuccessMessage('⚠️ هنوز گزارش فروش یا بارنامه‌ای برای اشتراک وجود ندارد');
      return;
    }
    ...
    try {
      ...
      pdf.addPage(
        pw.MultiPage(
          ...
          build: (context) => [
            pw.Center(
                child: _pdfShareTextWidget('بارنامه شماره ${manifest.number}', font,
                    fontSize: 24,
                    fontWeight: pw.FontWeight.bold,
                    color: PdfColors.blue,
                    textAlign: pw.TextAlign.center)),
            ...
            pw.Table(
              ...
              children: [
                ...
                ..._salesInvoices.asMap().entries.map((entry) {
                  ...
                  final customerName =
                      inv.customerName.trim().isEmpty ? 'نقدی' : inv.customerName.trim();
                  return pw.TableRow(
                    children: [
                      _pdfShareCell(_toPersianDigits(index.toString()), font),
                      _pdfShareCell(_toPersianDigits(inv.number.toString()), font),
                      _pdfShareCell(inv.productName, font, align: pw.TextAlign.right),
                      _pdfShareCell(_toPersianDigits(inv.quantity.toString()), font),
                      _pdfShareCell('${_formatPrice(inv.totalPrice)} ریال', font, fontSize: 8.5),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const ListTile(
                      ...
                      subtitle: Text('رویداد ثابت و غیرقابل ویرایش'),
                    ),
